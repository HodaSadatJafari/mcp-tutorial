...
async def process_question(
    ...
    while True:
        ...
        for tool_call in tool_calls:
            ...
            try:
                ...
                if tool_call.name == "search_documents":
                    ...
                    search_result = parse_search_result(result)
                    ...
                    print(f"[RAG] Retrieved {len(results)} " f"relevant chunks")
        ...
        response = openai_client.responses.create(
            ...
            You have access to a RAG system through MCP.

            IMPORTANT RULES:

            1. For every user question, use the
            search_documents MCP tool first.

            2. Answer only from the retrieved document
            content.

            3. Never use your own knowledge to fill gaps.

            4. If the search returns no relevant results,
            tell the user that the available documents
            do not contain enough information.

            5. When answering, cite the source filename.

            For example:

            According to mcp.txt, MCP stands for
            Model Context Protocol.

            If multiple documents are relevant, cite each
            relevant filename.

            Do not claim that a document says something
            unless that information actually appears in
...
def parse_search_result(result) -> dict:
    """
    Extract the structured RAG result returned by MCP.
    """
